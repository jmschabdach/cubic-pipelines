...
MPRLIST=$1
FSLVERSION=$2
BASE=/cbica/projects/bgdimagecentral/Projects/mpr_analysis   # Change this line to the appropriate project path
...
for fn in $(cat $MPRLIST) ; do
    ...
    # This part of the for loop adjusts some variables used in the reconall-job.sh script based on FSL version, etc.
    ...
    if [[ "$tmp" == *"mpr_preproc"* ]] ; then
        ...
        # This is the line that actually submits the job to the job manager (see reconall-job.sh for usage)
